...
Usage:
  tracefrugal report --trace run.jsonl --prices prices.json [--format json]
  tracefrugal compare --baseline before.jsonl --candidate after.jsonl --prices prices.json [--max-increase 5] [--format json]
...
func run(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if format != "text" && format != "json" {
		return fail(fmt.Errorf("format must be text or json"))
	}
	f, err := open(prices)
	if err != nil {
		return fail(err)
